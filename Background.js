...
// Função para obter localização aproximada pelo IP (caso queira usar)
async function getLocationByIP() {
  try {
    ...
    // Goiânia padrão
    return { lat: -16.6809, lon: -49.2539 };
  }
}
...
async function checkNearbyPlanes(userLocation) {
  try {
    ...
    const planes = data.states
      ...
      .filter((p) => p.distance <= 50); // até 50km
...
module.exports = { getLocationByIP, checkNearbyPlanes };
